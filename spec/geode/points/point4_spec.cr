...
Spectator.describe Geode::Point4 do
  subject(point) { Geode::Point4.new(3, 5, 7, 9) }

  it "stores the coordinates" do
    aggregate_failures do
      expect(point[0]).to eq(3)
      expect(point[1]).to eq(5)
      expect(point[2]).to eq(7)
      expect(point[3]).to eq(9)
    end
  end
  ...
  describe "#initialize" do
    context "with a block" do
      ...
      it "use the block value" do
        point = Geode::Point4(Int32).new &.itself
        expect(point).to eq(Geode::Point4[0, 1, 2, 3])
      end
    end

    context "with mismatched types" do
      it "converts the types" do
        point = Geode::Point4(Float64).new(1, 2, 3, 4)
        aggregate_failures do
          expect(point[0]).to eq(1.0)
          expect(point[1]).to eq(2.0)
          expect(point[2]).to eq(3.0)
          expect(point[3]).to eq(4.0)
        end
      end
    end
  end
  ...
  describe ".[]" do
    it "creates a point" do
      point = Geode::Point4[42, 24, 21, 12]
      aggregate_failures do
        expect(point[0]).to eq(42)
        expect(point[1]).to eq(24)
        expect(point[2]).to eq(21)
        expect(point[3]).to eq(12)
      end
    end

    it "casts types" do
      point = Geode::Point4(Float64)[2, 4, 6, 8]
      aggregate_failures do
        expect(point[0]).to be(2.0)
        expect(point[1]).to be(4.0)
        expect(point[2]).to be(6.0)
        expect(point[3]).to be(8.0)
      end
    end
  end

  describe ".zero" do
    subject { Geode::Point4(Int32).zero }

    it "returns a point with all zeroes" do
      is_expected.to eq(Geode::Point4[0, 0, 0, 0])
    end
  end

  describe ".origin" do
    subject { Geode::Point4(Int32).origin }
  ...
  describe "#w" do
    ...
    it "returns the w-coordinate" do
      is_expected.to eq(9)
    end
  end
  ...
  describe "#map" do
    it "transforms coordinates" do
      ...
      expect(mapped).to eq(Geode::Point4[6.0, 10.0, 14.0, 18.0])
    end
  end
  ...
  describe "#map_with_index" do
    ...
    it "provides an offset" do
      ...
      expect(mapped).to eq(Geode::Point4[0, 5, 14, 27])
    end

    it "applies the offset" do
      mapped = point.map_with_index(2) { |v, i| v * i }
      expect(mapped).to eq(Geode::Point4[6, 15, 28, 45])
    end
  end
  ...
  describe "#to_vector" do
    ...
    it "returns a vector" do
      is_expected.to eq(Geode::Vector4[3, 5, 7, 9])
    end
  end
  ...
  describe "#inspect" do
    ...
    it "is formatted correctly" do
      is_expected.to eq("Geode::Point4(Int32)#<x: 3, y: 5, z: 7, w: 9>")
    end
  end
  ...
  describe "#to_slice" do
    it "returns a slice containing the coordinates" do
      ...
      aggregate_failures do
        expect(slice[0]).to eq(3)
        expect(slice[1]).to eq(5)
        expect(slice[2]).to eq(7)
        expect(slice[3]).to eq(9)
      end
    end
