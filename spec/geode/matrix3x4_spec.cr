...
Spectator.describe Geode::Matrix3x4 do
  ...
  describe "#*(matrix)" do
    let(m1) { Geode::Matrix3x4[[2, 1, 2, 1], [4, 3, 4, 3], [6, 5, 6, 5]] }
    let(m2) { Geode::Matrix[[1, 10, 100], [2, 20, 200], [3, 30, 300], [4, 40, 400]] }

    it "multiplies matrices together" do
      expect(m1 * m2).to eq(Geode::Matrix[[14, 140, 1400], [34, 340, 3400], [54, 540, 5400]])
    end
  end
  ...
  context Geode::CommonMatrix do
    ...
    describe "#to_s" do
      ...
      it "is formatted correctly" do
        is_expected.to match(/^\[\[\d+, \d+, \d+, \d+\], \[\d+, \d+, \d+, \d+\], \[\d+, \d+, \d+, \d+\]\]$/)
      end
    end
  end
    ...
    describe "#scale" do
      ...
      context "with a scalar" do
        it "scales each element by the same amount" do
          expect(matrix.scale(5)).to eq(Geode::Matrix3x4[[10, 5, 10, 5], [20, 15, 20, 15], [30, 25, 30, 25]])
        end
      end
    end
