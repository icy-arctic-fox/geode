...
Spectator.describe Geode::Matrix1x4 do
  TOLERANCE = 0.000000000000001

  subject(matrix) { Geode::Matrix1x4[[3, 5, 7, 9]] }
  ...
  it "stores values for elements" do
    aggregate_failures do
      ...
      expect(matrix[0, 3]).to eq(9)
    end
  end

  describe "#initialize" do
    it "accepts a flat list of elements" do
      matrix = Geode::Matrix1x4.new({3, 5, 7, 9})
      aggregate_failures do
        expect(matrix[0, 0]).to eq(3)
        expect(matrix[0, 1]).to eq(5)
        expect(matrix[0, 2]).to eq(7)
    ...
    it "accepts another matrix" do
      ...
      matrix = Geode::Matrix1x4.new(other)
      aggregate_failures do
        expect(matrix[0, 0]).to eq(3)
        expect(matrix[0, 1]).to eq(5)
        expect(matrix[0, 2]).to eq(7)
  ...
  describe ".zero" do
    subject(zero) { Geode::Matrix1x4(Int32).zero }

    it "returns a zero matrix" do
      expect(zero).to eq(Geode::Matrix1x4[[0, 0, 0, 0]])
    end
  end
  ...
  describe "#map" do
    it "creates a matrix" do
      ...
      expect(mapped).to be_a(Geode::Matrix1x4(Int32))
    end

    it "uses the new values" do
      mapped = matrix.map { |e| e.to_f * 2 }
      expect(mapped).to eq(Geode::Matrix1x4[[6.0, 10.0, 14.0, 18.0]])
    end
  end
  ...
  describe "#transpose" do
    ...
    it "transposes the matrix" do
      is_expected.to eq(Geode::Matrix4x1[[3], [5], [7], [9]])
    end
  end

  describe "#*(matrix)" do
    let(m1) { Geode::Matrix1x4[[3, 5, 7, 9]] }
    subject { m1 * m2 }
    ...
    context "with a generic matrix" do
      ...
      it "multiplies matrices together" do
        is_expected.to eq(Geode::Matrix[[116, 140]])
      end
    end
    ...
    context "with a 4x1 matrix" do
      ...
      it "multiplies matrices together" do
        is_expected.to eq(Geode::Matrix[[70]])
      end
    end
    ...
    context "with a 4x2 matrix" do
      ...
      it "multiplies matrices together" do
        is_expected.to eq(Geode::Matrix[[70, 700]])
      end
    end
    ...
    context "with a 4x3 matrix" do
      ...
      it "multiplies matrices together" do
        is_expected.to eq(Geode::Matrix[[70, 700, 7000]])
      end
    end
    ...
    context "with a 4x4 matrix" do
      ...
      it "multiplies matrices together" do
        is_expected.to eq(Geode::Matrix[[70, 700, 7000, 70000]])
      end
    end
  end
  ...
  context Geode::CommonMatrix do
    ...
    describe "#each_indices" do
      it "yields each combination of indices" do
        ...
        expect(indices).to eq([{0, 0}, {0, 1}, {0, 2}, {0, 3}])
      end
    end
    ...
    describe "#each_with_indices" do
      it "yields each element with its indices" do
        ...
        expect(indices).to eq([{3, 0, 0}, {5, 0, 1}, {7, 0, 2}, {9, 0, 3}])
      end
    end

    describe "#map_with_index" do
      it "creates a Matrix1x4" do
      ...
      it "uses the new values" do
        ...
        expect(mapped).to eq(Geode::Matrix1x4[[0.0, 5.0, 14.0, 27.0]])
      end

      it "adds the offset" do
        mapped = matrix.map_with_index(3) { |e, i| e * i }
        expect(mapped).to eq(Geode::Matrix1x4[[9, 20, 35, 54]])
      end
    end
    ...
    describe "#map_with_indices" do
      ...
      it "uses the new values" do
        ...
        expect(mapped).to eq(Geode::Matrix1x4[[0.0, 1.0, 2.0, 3.0]])
      end
    end

    describe "#zip_map" do
      it "iterates two matrices" do
        m1 = Geode::Matrix1x4[[6, 4, 9, 2]]
        m2 = Geode::Matrix1x4[[3, 4, 3, 5]]
        result = m1.zip_map(m2) { |a, b| a // b }
        expect(result).to eq(Geode::Matrix1x4[[2, 1, 3, 0]])
      end
    end
    ...
    describe "#each_row" do
      it "enumerates each row" do
        ...
        expect(rows).to eq([Geode::Vector[3, 5, 7, 9]])
      end
    end
    ...
    describe "#each_row_with_index" do
      it "enumerates each row" do
        ...
        expect(rows).to eq([{Geode::Vector[3, 5, 7, 9], 0}])
      end

      it "applies the offset" do
        rows = [] of Tuple(Geode::CommonVector(Int32, 4), Int32)
        matrix.each_row_with_index(5) { |row, i| rows << {row, i} }
        expect(rows).to eq([{Geode::Vector[3, 5, 7, 9], 5}])
      end
    end
    ...
    describe "#each_column" do
      it "enumerates each column" do
        ...
        expect(columns).to eq([Geode::Vector[3], Geode::Vector[5], Geode::Vector[7], Geode::Vector[9]])
      end
    end
    ...
    describe "#each_column_with_index" do
      it "enumerates each column" do
        ...
        expect(columns).to eq([{Geode::Vector[3], 0}, {Geode::Vector[5], 1}, {Geode::Vector[7], 2}, {Geode::Vector[9], 3}])
      end

      it "applies the offset" do
        columns = [] of Tuple(Geode::CommonVector(Int32, 1), Int32)
        matrix.each_column_with_index(3) { |column, i| columns << {column, i} }
        expect(columns).to eq([{Geode::Vector[3], 3}, {Geode::Vector[5], 4}, {Geode::Vector[7], 5}, {Geode::Vector[9], 6}])
      end
    end
    ...
    describe "#row" do
      it "returns the correct elements" do
        expect(matrix.row(0)).to eq(Geode::Vector[3, 5, 7, 9])
      end
    ...
    describe "#rows_at" do
      it "returns the correct rows" do
        expect(matrix.rows_at(0)).to eq({Geode::Vector[3, 5, 7, 9]})
      end
    end
  ...
  context Geode::MatrixComparison do
    ...
    describe "#zero?" do
      ...
      context "with a zero matrix" do
        let(matrix) { Geode::Matrix1x4(Int32).zero }
    ...
    describe "#near_zero?" do
      ...
      context "with elements within tolerance of zero" do
        let(matrix) { Geode::Matrix1x4[[0.05, 0.1, 0.0, 0.001]] }
    ...
    describe "#==" do
      ...
      context "with a nxm-dimension matrix" do
        context "with equal values" do
          let(other) { Geode::Matrix1x4[[3, 5, 7, 9]] }
        ...
        context "with unequal values" do
          let(other) { Geode::Matrix1x4[[9, 7, 5, 3]] }
  ...
  context Geode::MatrixIterators do
    describe "#each_indices" do
      ...
      it "iterates through each pair of indices" do
        expect(subject.to_a).to eq([{0, 0}, {0, 1}, {0, 2}, {0, 3}])
      end
    end
    ...
    describe "#each_with_indices" do
      ...
      it "iterates through each element and its indices" do
        expect(subject.to_a).to eq([{3, 0, 0}, {5, 0, 1}, {7, 0, 2}, {9, 0, 3}])
      end
    end
  ...
  context Geode::MatrixOperations do
    describe "#abs" do
      it "returns the absolute value" do
        expect(Geode::Matrix1x4[[3, -5, 0, 1]].abs).to eq(Geode::Matrix1x4[[3, 5, 0, 1]])
      end
    end

    describe "#abs2" do
      it "returns the absolute value squared" do
        expect(Geode::Matrix1x4[[3, -5, 0, 1]].abs2).to eq(Geode::Matrix1x4[[9, 25, 0, 1]])
      end
    end

    describe "#round" do
      it "rounds the elements" do
        expect(Geode::Matrix1x4[[1.2, -5.7, 1.5, 0.0]].round).to eq(Geode::Matrix1x4[[1.0, -6.0, 2.0, 0.0]])
      end

      context "with digits" do
        it "rounds the elements" do
          expect(Geode::Matrix1x4[[1.25, -5.77, 1.5, 0.0]].round(1)).to eq(Geode::Matrix1x4[[1.2, -5.8, 1.5, 0.0]])
        end
      end
    end
    ...
    describe "#sign" do
      it "returns the sign of each element" do
        expect(Geode::Matrix1x4[[5, -5, 0, 1]].sign).to eq(Geode::Matrix1x4[[1, -1, 0, 1]])
      end
    end

    describe "#ceil" do
      it "returns the elements rounded up" do
        expect(Geode::Matrix1x4[[1.2, -5.7, 1.5, 0.0]].ceil).to eq(Geode::Matrix1x4[[2.0, -5.0, 2.0, 0.0]])
      end
    end

    describe "#floor" do
      it "returns the elements rounded down" do
        expect(Geode::Matrix1x4[[1.2, -5.7, 1.5, 0.0]].floor).to eq(Geode::Matrix1x4[[1.0, -6.0, 1.0, 0.0]])
      end
    end

    describe "#fraction" do
      it "returns the fraction part of each element" do
        fraction = Geode::Matrix1x4[[1.2, -5.7, 0.5, 0.0]].fraction
        aggregate_failures do
    ...
    describe "#clamp" do
      context "with a min and max matrices" do
        it "restricts elements" do
          min = Geode::Matrix1x4[[-1, -2, -3, -4]]
          max = Geode::Matrix1x4[[1, 2, 3, 4]]
          expect(Geode::Matrix1x4[[-2, 3, 1, 0]].clamp(min, max)).to eq(Geode::Matrix1x4[[-1, 2, 1, 0]])
        end
      end
      ...
      context "with a min and max" do
        it "restricts elements" do
          expect(Geode::Matrix1x4[[-2, 2, 0, 1]].clamp(-1, 1)).to eq(Geode::Matrix1x4[[-1, 1, 0, 1]])
        end
      end
    ...
    describe "#edge" do
      context "with a scalar value" do
        it "returns correct zero and one elements" do
          expect(matrix.edge(6)).to eq(Geode::Matrix1x4[[0, 0, 1, 1]])
        end
      end

      context "with a matrix" do
        it "returns correct zero and one elements" do
          expect(matrix.edge(Geode::Matrix1x4[[2, 9, 7, 0]])).to eq(Geode::Matrix1x4[[1, 0, 1, 1]])
        end
      end
    end
    ...
    describe "#scale" do
      context "with a matrix" do
        let(other) { Geode::Matrix1x4[[3, 4, 5, 6]] }

        it "scales each element separately" do
          expect(matrix.scale(other)).to eq(Geode::Matrix1x4[[9, 20, 35, 54]])
        end
      end

      context "with a scalar" do
        it "scales each element by the same amount" do
          expect(matrix.scale(5)).to eq(Geode::Matrix1x4[[15, 25, 35, 45]])
        end
      end
    end
    ...
    describe "#lerp" do
      let(m1) { Geode::Matrix1x4[[3.0, 5.0, 7.0, 9.0]] }
      let(m2) { Geode::Matrix1x4[[13.0, 25.0, 37.0, 49.0]] }
      ...
      it "returns m1 when t = 0" do
        ...
        aggregate_failures do
          ...
          expect(matrix[0, 2]).to be_within(TOLERANCE).of(7.0)
          expect(matrix[0, 3]).to be_within(TOLERANCE).of(9.0)
        end
      end
      ...
      it "returns m2 when t = 1" do
        ...
        aggregate_failures do
          ...
          expect(matrix[0, 2]).to be_within(TOLERANCE).of(37.0)
          expect(matrix[0, 3]).to be_within(TOLERANCE).of(49.0)
        end
      end
      ...
      it "returns a mid-value" do
        ...
        aggregate_failures do
          ...
          expect(matrix[0, 2]).to be_within(TOLERANCE).of(19.0)
          expect(matrix[0, 3]).to be_within(TOLERANCE).of(25.0)
        end
      end
    end
    ...
    describe "#- (negation)" do
      it "negates the matrix" do
        expect(-Geode::Matrix1x4[[3, -3, 0, 1]]).to eq(Geode::Matrix1x4[[-3, 3, 0, -1]])
      end
    end

    describe "#+" do
      it "adds two matrices" do
        m1 = Geode::Matrix1x4[[5, 7, 9, 0]]
        m2 = Geode::Matrix1x4[[2, 4, -3, 1]]
        expect(m1 + m2).to eq(Geode::Matrix1x4[[7, 11, 6, 1]])
      end
    end
    ...
    describe "#-" do
      it "subtracts two matrices" do
        m1 = Geode::Matrix1x4[[5, 7, -3, 1]]
        m2 = Geode::Matrix1x4[[2, 5, -4, 1]]
        expect(m1 - m2).to eq(Geode::Matrix1x4[[3, 2, 1, 0]])
      end
    end
    ...
    describe "#*(number)" do
      it "scales a matrix" do
        expect(matrix * 3).to eq(Geode::Matrix1x4[[9, 15, 21, 27]])
      end
    end
    ...
    describe "#/" do
      it "scales a matrix" do
        matrix = Geode::Matrix1x4[[4.0, 6.0, 8.0, 10.0]]
        expect(matrix / 2).to eq(Geode::Matrix1x4[[2.0, 3.0, 4.0, 5.0]])
      end
    end

    describe "#//" do
      it "scales the matrix" do
        matrix = Geode::Matrix1x4[[4, 6, 8, 10]]
        expect(matrix // 2).to eq(Geode::Matrix1x4[[2, 3, 4, 5]])
      end
    end
  end
