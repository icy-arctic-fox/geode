...
Spectator.describe Geode::Matrix3x3 do
  ...
  describe ".identity" do
    subject(identity) { Geode::Matrix3x3(Int32).identity }

    it "returns an identity matrix" do
      expect(identity).to eq(Geode::Matrix3x3[[1, 0, 0], [0, 1, 0], [0, 0, 1]])
    end
  end

  describe ".new(scalar)" do
    subject(square) { Geode::Matrix3x3(Int32).new(7) }

    it "returns a matrix with the diagonal filled" do
      expect(square).to eq(Geode::Matrix3x3[[7, 0, 0], [0, 7, 0], [0, 0, 7]])
    end
  end
  ...
  describe "#sub" do
    let(matrix) { Geode::Matrix3x3[[1, 2, 3], [4, 5, 6], [7, 8, 9]] }
  ...
  describe "#*(matrix)" do
    let(m1) { Geode::Matrix3x3[[3, 5, 7], [4, 6, 8], [1, 3, 9]] }
    subject { m1 * m2 }
    ...
    context "with a generic matrix" do
      ...
      it "multiplies matrices together" do
        is_expected.to eq(Geode::Matrix[[53, 68], [62, 80], [55, 68]])
      end
    end
    ...
    context "with a 3x1 matrix" do
      ...
      it "multiplies matrices together" do
        is_expected.to eq(Geode::Matrix[[34], [40], [34]])
      end
    end
    ...
    context "with a 3x2 matrix" do
      ...
      it "multiplies matrices together" do
        is_expected.to eq(Geode::Matrix[[34, 340], [40, 400], [34, 340]])
      end
    end
    ...
    context "with a 3x3 matrix" do
      ...
      it "multiplies matrices together" do
        is_expected.to eq(Geode::Matrix[[34, 340, 3400], [40, 400, 4000], [34, 340, 3400]])
      end
    end
    ...
    context "with a 3x4 matrix" do
      ...
      it "multiplies matrices together" do
        is_expected.to eq(Geode::Matrix[[34, 340, 3400, 34000], [40, 400, 4000, 40000], [34, 340, 3400, 34000]])
      end
    end
  end
  ...
  context Geode::SquareMatrix do
    ...
    describe "#inverse" do
      ...
      context "with an invertible matrix" do
        let(matrix) { Geode::Matrix3x3[[4, 7, 3], [2, 6, 5], [1, 0, 9]] }

        it "returns an inverted matrix" do
          expect_within_tolerance(subject.not_nil!,
            0.504672897, -0.588785047, 0.158878505,
            -0.121495327, 0.308411215, -0.130841121,
            -0.056074766, 0.065420561, 0.093457944)
        end
      end

      context "with a non-invertible matrix" do
        let(matrix) { Geode::Matrix3x3[[2, 3, 4], [4, 6, 8], [6, 9, 12]] }
  ...
  context Geode::Matrix3x3Transforms3DConstructors do
    TOLERANCE = 0.000000001
    SQRT2     = Math.sqrt(2)
  ...
  context Geode::Matrix3x3Transforms3D do
    TOLERANCE = 0.000000001
    SQRT8     = Math.sqrt(8)

    let(vector) { Geode::Vector3[1, 1, 1] }
    let(original) { Geode::Matrix3[[2, 0, 0], [0, 2, 0], [0, 0, 2]] }
    let(transformed) { vector * matrix }
    ...
    describe "#translate" do
      let(vector) { Geode::Vector4[1, 2, 3, 1] }
      let(matrix) { original.translate(3, 4, 5) }

      it "translates the vector" do
        aggregate_failures do
          expect(transformed.x).to eq(5)
          expect(transformed.y).to eq(8)
          expect(transformed.z).to eq(11)
  ...
  context Geode::Matrix3x3Transforms2DConstructors do
    let(vector) { Geode::Vector3[1, 1, 1] }
    let(transformed) { vector * matrix }

    describe ".rotate" do
      let(matrix) { Geode::Matrix3(Float64).rotate(45.degrees) }

      it "rotates a vector" do
        aggregate_failures do
          expect(transformed.x).to be_within(TOLERANCE).of(0)
          expect(transformed.y).to be_within(TOLERANCE).of(Math.sqrt(2))
    ...
    describe ".rotate90" do
      let(matrix) { Geode::Matrix3(Float64).rotate90 }
    ...
    describe ".rotate180" do
      let(matrix) { Geode::Matrix3(Float64).rotate180 }
    ...
    describe ".rotate270" do
      let(matrix) { Geode::Matrix3(Float64).rotate270 }
    ...
    describe ".scale2(amount)" do
      let(matrix) { Geode::Matrix3(Float64).scale2(3) }
    ...
    describe ".scale(x, y)" do
      let(matrix) { Geode::Matrix3(Float64).scale(2, 3) }
    ...
    describe ".reflect_xy" do
      let(matrix) { Geode::Matrix3(Float64).reflect_xy }
    ...
    describe ".shear_x" do
      let(vector) { Geode::Vector3[2, 3, 1] }
      let(matrix) { Geode::Matrix3(Float64).shear_x(3) }
    ...
    describe ".shear_y" do
      let(vector) { Geode::Vector3[2, 3, 1] }
      let(matrix) { Geode::Matrix3(Float64).shear_y(3) }
  ...
  context Geode::Matrix3x3Transforms2D do
    let(vector) { Geode::Vector3[1, 1, 1] }
    let(original) { Geode::Matrix3[[2, 0, 0], [0, 2, 0], [0, 0, 1]] }
    let(transformed) { vector * matrix }

    describe "#rotate" do
      let(matrix) { original.rotate(135.degrees) }

      it "rotates a vector" do
        aggregate_failures do
          expect(transformed.x).to be_within(TOLERANCE).of(-Math.sqrt(8))
          expect(transformed.y).to be_within(TOLERANCE).of(0)
          expect(transformed.z).to be_within(TOLERANCE).of(1)
        end
      end
    ...
    describe "#reflect_xy" do
      let(matrix) { original.reflect_xy }
    ...
    describe "#translate" do
      let(vector) { Geode::Vector3[2, 3, 1] }
      let(matrix) { original.translate(4, 5) }
